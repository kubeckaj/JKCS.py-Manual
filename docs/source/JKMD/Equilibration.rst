...
However, if you want to submit the job to the cluster, it make sense that you immediately run the two simulation in series. Therefore, the argument :guilabel:`-follow` allows you the start completely separate simulation. Note, that all parameters from the previous simulation are used unless redefined after the :guilabel:`-follow` argument. You can run the above like this:
